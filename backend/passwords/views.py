...
def password_history(request):
    action_key = {"GET": None, "POST": "import", "DELETE": "clear"}[request.method]
    auth_error = require_feature_permission(request, "password", action_key, "没有密码生成器操作权限")
    if auth_error:
        return auth_error

    if request.method == "DELETE":
        PasswordRecord.objects.filter(created_by=request.user).delete()
    ...
    return Response(PasswordRecordSerializer(PasswordRecord.objects.filter(created_by=request.user)[:16], many=True).data)
