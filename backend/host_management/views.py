...
def host_group_detail(request, group_id: int):
    ...
    if request.method == "PUT":
        ...
        if moving:
            ...
            reorder_group(group, parent, str(request.data.get("position", "after")), request.data.get("target"))
        else:
            group.save(update_fields=["name"])

        return Response(
            {
                "group": HostGroupSerializer(group, context={"counts": {group.id: group.hosts.count()}}).data,
                "groups": groups_payload(),
            }
        )

    if ManagedHost.objects.filter(group_id__in=collect_descendant_ids(group)).exists():
        return bad_request("请先删除该分组下所有主机")
    group.delete()
...
def host_management_export(_request):
    return Response(export_host_management_payload())
