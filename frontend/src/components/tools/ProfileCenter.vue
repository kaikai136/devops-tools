...
<template>
  <section v-if="activeTool === 'profile'" class="profile-center-page">
    <NativeAlert v-if="message" class="profile-message" type="error" :closable="false" :title="message" />
    <NativeAlert v-if="isLoading" class="profile-message info" type="info" :closable="false" title="正在加载个人资料..." />
    ...
    <aside class="profile-overview-card">
      ...
      <div class="profile-identity">
        <div class="profile-name-row">
          ...
          <div class="profile-role-tags">
            <NativeTag v-for="tag in roleTags" :key="tag" size="small" effect="plain">{{ tag }}</NativeTag>
          </div>
        </div>
      ...
      <NativeButton class="profile-logout-button" @click="logout">
    ...
    <div v-if="canUseAnyPageAction('profile', ['edit', 'avatar', 'password', '2fa_enable', '2fa_disable'])" class="profile-settings-stack">
        ...
        <div class="profile-basic-grid" :class="{ 'without-avatar': !canUploadAvatar }">
          <section v-if="canUploadAvatar" class="profile-avatar-editor">
            ...
            <div>
              <strong>资料头像</strong>
              <p>上传图片后会立即更新当前头像。</p>
              <NativeButton class="profile-secondary-button" :loading="isUploadingAvatar" @click="triggerAvatarUpload">
          ...
          <form v-if="canEditProfile" class="profile-form-grid" @submit.prevent="saveProfile">
            <label>
              <span>用户名</span>
              <NativeInput v-model.trim="profileForm.username" autocomplete="username" />
            </label>
            <label>
              <span>显示名</span>
              <NativeInput v-model.trim="profileForm.first_name" placeholder="例如：运维船长" />
            </label>
            <label>
              <span>邮箱</span>
              <NativeInput v-model.trim="profileForm.email" type="email" autocomplete="email" placeholder="name@example.com" />
            </label>
            <div class="profile-actions">
              <NativeButton class="profile-primary-button" native-type="submit" type="primary" :loading="isSavingProfile">
      ...
      <section v-if="canChangePassword" class="profile-panel">
        ...
        <form class="profile-form-grid" @submit.prevent="changePassword">
          <label>
            <span>当前密码</span>
            <NativeInput v-model="passwordForm.currentPassword" type="password" autocomplete="current-password" show-password />
          </label>
          <label>
            <span>新密码</span>
            <NativeInput v-model="passwordForm.newPassword" type="password" autocomplete="new-password" show-password />
          </label>
          <label>
            <span>确认新密码</span>
            <NativeInput v-model="passwordForm.confirmPassword" type="password" autocomplete="new-password" show-password />
          </label>
          ...
          <NativeAlert v-if="passwordMismatch" class="profile-inline-error" type="warning" :closable="false" title="两次输入的新密码不一致。" />
          <div class="profile-actions">
            <NativeButton class="profile-primary-button" native-type="submit" type="primary" :disabled="!canSubmitPassword" :loading="isChangingPassword">
      ...
      <section v-if="(twoFactorEnabled && canDisableTwoFactor) || (!twoFactorEnabled && canEnableTwoFactor)" class="profile-panel profile-security-panel">
        <header>
          ...
          <NativeTag class="profile-status-pill" :type="twoFactorEnabled ? 'success' : 'info'" effect="plain">{{ twoFactorEnabled ? '已启用' : '未启用' }}</NativeTag>
        </header>

        <div v-if="!twoFactorEnabled" class="profile-2fa-setup">
          <NativeButton class="profile-primary-button" type="primary" :loading="isPreparingTwoFactor" @click="prepareTwoFactor">
          ...
          <div v-if="setupPayload" class="profile-qr-setup">
            ...
            <div>
              ...
              <NativeButton @click="copyText(setupPayload.secret, '密钥已复制。')">复制密钥</NativeButton>
            </div>
          </div>

          <form v-if="setupPayload" class="profile-2fa-form" @submit.prevent="confirmTwoFactor">
            <NativeInput v-model.trim="twoFactorCode" inputmode="numeric" maxlength="6" placeholder="输入 6 位验证码" />
            <NativeButton class="profile-primary-button" native-type="submit" type="primary" :disabled="twoFactorCode.length !== 6" :loading="isConfirmingTwoFactor">
        ...
        <form v-else class="profile-2fa-form danger-zone" @submit.prevent="disableTwoFactor">
          <NativeInput v-model="disablePassword" type="password" autocomplete="current-password" placeholder="当前密码" show-password />
          <NativeInput v-model.trim="disableCode" inputmode="numeric" maxlength="6" placeholder="6 位验证码" />
          <NativeButton class="profile-danger-button" native-type="submit" type="danger" :disabled="!disablePassword || disableCode.length !== 6" :loading="isDisablingTwoFactor">
